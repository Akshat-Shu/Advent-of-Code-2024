...
void checkValue(int i, int j, int val, vector<string> &s, int n_line, int n_char, set<pair<int, int>> &found) {
    ...
    {
        checkValue(i-1, j, val+1, s, n_line, n_char, found);
        checkValue(i+1, j, val+1, s, n_line, n_char, found);
        checkValue(i, j-1, val+1, s, n_line, n_char, found);
        checkValue(i, j+1, val+1, s, n_line, n_char, found);
    }
}
...
int main(int argc, char const *argv[]) {
    ...
    for(auto zero: zeroes) {
        ...
        checkValue(zero.first, zero.second, 0, s, n_line, n_char, found);
        ans += found.size();
        if(found.size()) {
